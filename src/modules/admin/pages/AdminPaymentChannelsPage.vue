...
async function handleCreate() {
  ...
  try {
    ...
    actionMessage.value = `支付通道 ${response.channel.name} 已创建。`;
    closeCreateModal();
    await loadChannels();
  } catch (error) {
...
async function handleUpdate() {
  ...
  try {
    ...
    updateChannelInList(response.channel);
    actionMessage.value = `支付通道 ${response.channel.name} 已更新。`;
    closeEditModal();
  } catch (error) {
...
async function handleToggle() {
  ...
  try {
    ...
    updateChannelInList(response.channel);
    actionMessage.value = `支付通道 ${response.channel.name} 已${toggleNextState.value ? '启用' : '停用'}。`;
    closeToggleModal();
  } catch (error) {
